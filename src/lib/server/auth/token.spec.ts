...
const testSession: Record<string, any> = {
...
describe("createToken", () => {
	it("returns a three-part JWT string", () => {
		const token = createToken(testSession as any);
	...
	it("payload contains sub, exp, email, session_id, session_exp, platform_role", () => {
		...
		expect(decoded.exp).toBeGreaterThan(Math.floor(Date.now() / 1000) + 85000);
	});

	it("generates unique tokens with different session IDs", () => {
		const t1 = createToken(testSession as any);
		const t2 = createToken({
			...testSession,
			sessionId: "sess_02JN6K3R4X5Y7Z9ABCDEFGHIJK",
		} as any);
...
describe("validateToken", () => {
	it("validates a freshly created token", () => {
		const token = createToken(testSession as any);
		const result = validateToken(token) as any;
	...
	it("rejects a token with a tampered payload", () => {
		...
		const result = validateToken(tamperedToken) as any;
	...
	it("rejects a token missing the signature part", () => {
		const token = createToken(testSession as any);
		const parts = token.split(".");
		const shortToken = [parts[0], parts[1]].join(".");
		const result = validateToken(shortToken) as any;
		expect(result.valid).toBe(false);
		expect(result.error).toBe("unsigned");
	});

	it("rejects a completely malformed token", () => {
		const result = validateToken("not-a-jwt") as any;
		expect(result.valid).toBe(false);
	});

	it("rejects an empty string", () => {
		const result = validateToken("") as any;
		expect(result.valid).toBe(false);
	...
	it("rejects token with malformed payload (non-JSON)", () => {
		...
		const result = validateToken(badToken) as any;
		expect(result.valid).toBe(false);
		expect(result.error).toBe("payload");
	});

	it("rejects token with invalid payload fields (missing email)", () => {
		const [header] = createToken(testSession as any).split(".");
...
describe("signature tamper returns sessionId", () => {
	it("returns the session_id from the payload when signature is invalid", () => {
		...
		const result = validateToken(tampered) as any;
		expect(result.valid).toBe(false);
