<script lang="ts">
	...
	export let board: import("$types/frontend/kanban").Board;

	const flipDurationMs = 200;

	function handleDndConsiderColumns(
		e: CustomEvent<DndEvent<import("$types/frontend/kanban").Column>>,
	) {
	...
	function handleDndFinalizeColumns(
		...
		data.append(
			...
			JSON.stringify(
				board.columns.map(
					(c: import("$types/frontend/kanban").Column, i: number) => ({
						id: c.id,
						order: i,
					}),
				),
	...
	function handleCardDrop(
		...
		data.append(
			"items",
			JSON.stringify(
				items.map((c: import("$types/frontend/kanban").Card, i: number) => ({
					id: c.id,
					order: i,
				})),
			),
...
<div class="flex h-full flex-col">
		...
		{#if isAddingColumn}
			...
			>
				...
				<button
					...
					on:click={() => (isAddingColumn = false)}>Cancel</button
				>
			</form>
		{:else}
			<button
	...
	<div
		class="flex flex-1 items-start overflow-x-auto overflow-y-hidden p-4 whitespace-nowrap"
